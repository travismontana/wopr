...
from lib.basic_functions import debugit, setup_logger
from lib.models_lib import check_data, download_model, get_model_status
...
def model_main_display():
    ...
    for model in models:
        ...
        cols[1].write(
            next(
                (f["name"] for f in model_families if f["id"] == model["familyid"]), ""
            )
        )
        if downloaded:
        ...
        with cols[3]:
            ...
            if button_cols[0].button(
                "Download", key=f"dl_{model['shortname']}", disabled=downloaded
            ):
            ...
            if button_cols[1].button("Backup", key=f"bak_{model['shortname']}"):
                st.toast(f"Backing up {model['shortname']}")
                # Do the thing

            if button_cols[2].button("Config", key=f"cfg_{model['shortname']}"):
