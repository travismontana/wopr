...
def workplayer(playername, playerkind):
    ...
    log.info(
        f"Player OK id={player.get('id')} name={player.get('name')} isbot={player.get('isbot')}"
    )
    return player
...
if not st.session_state.start_game_info:
    ...
    num_players = st.number_input(
        "Total Players (including yourself):", min_value=2, max_value=6, value=3, step=1
    )
        ...
        for i in range(2, int(num_players) + 1):
            ...
            with col1:
                p_name = st.text_input(
                    "Name/Character:", f"player{i}", key=f"p{i}_name"
                )
            with col2:
                p_type = st.selectbox("Type:", ["bot", "human"], key=f"p{i}_type")
    ...
    names = ", ".join(
        [player["name"] for player in st.session_state.start_game_info.values()]
    )
    ...
    if st.session_state.session_uuid is None:
        ...
        selected_game_name = st.selectbox(
            "Select a game:", options=game_names, key="game_selectbox"
        )
        ...
        if st.button("Start New Game", key="start_new_game_button"):
            log.info(
                f"Start New Game clicked game_id={selected_game.get('id')} game_name={selected_game.get('name')}"
            )
            st.session_state.session_uuid, st.session_state.session_id = newSession(
                selected_game["id"]
            )
    ...
    # Game in Progress
        ...
        st.write(
            f"Round: {st.session_state.current_round} / {game.get('max_rounds', 'Unknown')}"
        )

        # Round hasn't started yet
        if not st.session_state.round_started:
            if st.button(
                f"Start Round {st.session_state.current_round}",
                key=f"start_round_{st.session_state.current_round}",
            ):
                ...
                try:
                    ...
                    st.success(
                        f"Round {st.session_state.current_round} started - Initial state captured"
                    )
        ...
        # Round in progress
            ...
            if st.session_state.current_round_play >= len(player_keys):
                if st.button(
                    f"End Round {st.session_state.current_round}",
                    key=f"end_round_{st.session_state.current_round}",
                ):
                    log.info(
                        f"End Round clicked round={st.session_state.current_round}"
                    )
                    try:
                        ...
                        st.success(
                            f"Round {st.session_state.current_round} ended - Final state captured"
                        )
                        ...
                        if st.session_state.current_round >= max_rounds:
                            ...
                            if st.button(
                                "Start New Game", key="start_new_game_after_complete"
                            ):
                                ...
                                st.session_state.turn_token = None
                                st.session_state.turn_phrase = random.choice(
                                    PLAYPHRASES
                                )
                                st.session_state.playnote = ""
                            ...
                            log.info(
                                f"Advance to next round round={st.session_state.current_round}"
                            )
                            st.rerun()
            ...
            # Player turn
                ...
                if st.session_state.turn_token != turn_token:
                    ...
                    st.session_state.playnote = ""
                    log.info(
                        f"New turn token={turn_token} player={current_player.get('name')}"
                    )

                st.info(
                    f"Round {st.session_state.current_round} - {current_player['name']}'s turn (Play {play_num})"
                )
                ...
                with st.form("play_form", enter_to_submit=False):
                    ...
                    if submitted:
                        log.info(
                            f"Play submitted turn_token={turn_token} player={current_player.get('name')} play_num={play_num}"
                        )
                        try:
                            ...
                            st.success(
                                f"Round {st.session_state.current_round} - {current_player['name']}'s play captured"
                            )
                            st.session_state.current_round_play += 1
                            log.info(
                                f"Advance to next play current_round_play={st.session_state.current_round_play}"
                            )
                            st.rerun()
