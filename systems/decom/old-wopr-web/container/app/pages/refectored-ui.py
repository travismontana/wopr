...
def fetch_label_studio_state(game_id: int) -> dict:
    ...
    for project in ls_projects.get("results", []):
        ...
        if match and int(match.group(1)) == game_id:
            proj_id = project["id"]
            log.info(
                f"Found matching Label Studio project: {project['title']} (ID: {proj_id})"
            )
...
def update_session_status(session_id: int, status: str) -> dict:
    ...
    try:
        ...
        return {"status": "success", "message": f"Session {session_id} set to {status}"}
    except Exception as e:
...
def process_file_presence(raw_presence: dict) -> dict:
    ...
    log.info(
        f"Processed file presence: {total} files across {len(locations)} locations"
    )
...
def process_label_tasks(tasks: list, session_uuid: str) -> dict:
    ...
    """
    log.info(
        f"Processing {len(tasks)} Label Studio tasks for session UUID: {session_uuid}"
    )
...
def execute_archive_operation(session_id: int, plays: list, direction: str) -> dict:
    ...
    file_operation = (
        archive_session_files if direction == "archive" else unarchive_session_files
    )
    ...
    if play_result.get("status") not in ["success", "partial"]:
        log.error("Play record update failed")
        return results
...
def display_archive_controls(session_id: int, plays: list, current_status: str):
    ...
    # Second click: show confirmation
        ...
        with col1:
            if st.button(
                f"✓ Yes, {operation.title()}",
                type="primary",
                key=f"confirm_yes_{operation}",
            ):
...
def existing_session():
    ...
    if current_status != "archived":
            ...
            with st.spinner("Checking Label Studio..."):
                ...
                if ls_project:
                    task_data = process_label_tasks(
                        ls_project.get("tasks", []), session_uuid
                    )
        ...
        st.info(
            "📦 Session is archived. Unarchive to view file and Label Studio status."
        )
