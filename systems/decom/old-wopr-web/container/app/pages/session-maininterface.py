...
def session_management():
    ...
    session_info = (
        get_one("session", current_session_id) if current_session_id else None
    )
    ...
    game_name = (
        games[game_id]["name"] if game_id and game_id < len(games) else "Unknown"
    )
        ...
        # Confirmation state: show warning and confirm button
            ...
            with col1:
                if st.button("✓ Yes, Archive", type="primary"):
                    ...
                    image_archive_results = queue_session_task(
                        current_session_id, "archive"
                    )
                    ...
                    log.info(
                        f"Image archive task completed with results: {image_archive_results}"
                    )
                    db_archive_results = archive_play_records(current_session_id, plays)
                    log.info(
                        f"Database archive completed with results: {db_archive_results}"
                    )
                    session_archive_results = archive_session(current_session_id)
                    log.info(
                        f"Session archive completed with results: {session_archive_results}"
                    )
                    st.success("Archived!")
                    st.session_state.confirm_archive = False
                    st.rerun()
...
def play_walkthrough():
    log.info(
        f"Starting Play Walkthrough with {len(plays)} plays Players: {len(players)}."
    )
    ...
    for play in plays:
        playername = next(
            (player["name"] for player in players if player["id"] == play["playerid"]),
            "Unknown Player",
        )
...
def process_management():
    ...
    st.write(
        f":gray[Load into project dataset status:] {load_into_project_dataset_status}"
    )
    ...
    for project in ls_projects.get("results", []):
        ...
        if match:
            ...
            if project_game_id == target_game_id:
                ...
                log.info(
                    f"Found matching project: {project['title']} (ID: {project['id']})"
                )
                break
...
def process_proj_tasks(proj_tasks):
    ...
    for task in proj_tasks:
        ...
        simplified_tasks.append(
            {"task_id": task_id, "filename": filename, "is_labeled": task_is_labeled}
        )
...
def process_states_display(session_info, process_states, file_presence):
    ...
    proj_tasks_queue_total = (
        process_states.get("queue_total") if process_states else None
    )
    proj_tasks_queue_done = process_states.get("queue_done") if process_states else None
    proj_tasks_finished_tasks = (
        process_states.get("finished_task_number") if process_states else None
    )
    ...
    is_game_in_tasks = is_game_in_tasks_check(
        simplified_tasks, session_info.get("UUID")
    )
    if debug:
        st.write("is_game_in_tasks")
        st.json(is_game_in_tasks)
    num_session_in_tasks = (
        len(is_game_in_tasks) if isinstance(is_game_in_tasks, list) else 0
    )
    st.write(
        f"Number of images from this session in the project tasks: {num_session_in_tasks}"
    )
...
def is_game_in_tasks_check(simplified_tasks, session_uuid):
    ...
    for task in simplified_tasks:
        if session_uuid in task["filename"]:
            ...
            results.append({"filename": task["filename"], "task_id": task["task_id"]})
    if results:
        return results
    else:
...
def existing_session():
    ...
    log.info(
        f"Session selected in UI: {st.session_state.selected_session} with ID {st.session_state.selected_session_id}"
    )
