...
st.write(
    "Checking ML training image coverage across pieces, positions, and lighting conditions."
)
...
st.write(
    f"- Center position coverage: {len(intensities)} intensities × {len(color_temps)} temps = {len(intensities) * len(color_temps)} combinations"
)
st.write("- Random position: >3 images")
...
for piece in pieces:
    ...
    for pos_label, pos_value in positions.items():
        matching = [
            img for img in piece_images if img.get("object_position") == pos_value
        ]
    ...
    center_images = [
        img for img in piece_images if img.get("object_position") == "center"
    ]
    center_combos = set()
    ...
    required_combos = {
        (intensity, temp) for intensity in intensities for temp in color_temps
    }
    ...
    random_images = [
        img for img in piece_images if img.get("object_position") == "random"
    ]
    ...
    piece_status.append(
        {
            ...
            else f"✗ Missing: {', '.join(missing_positions)}",
            "Center Coverage": "✓"
            if center_complete
            else f"✗ Missing {len(missing_combos)} combos",
            "Random Count": f"✓ ({random_count})"
            if random_complete
            else f"✗ ({random_count})",
...
filter_choice = st.radio(
    "Show:", ["All Pieces", "Complete Only", "Incomplete Only"], horizontal=True
)
...
selected_piece_name = st.selectbox(
    "Select piece for details:", [p["name"] for p in pieces]
)
selected_piece = next(p for p in pieces if p["name"] == selected_piece_name)
selected_piece_images = [
    img for img in mlimages if img.get("piece_id") == selected_piece["id"]
]
...
with tab1:
    ...
    for pos_label, pos_value in positions.items():
        count = len(
            [
                img
                for img in selected_piece_images
                if img.get("object_position") == pos_value
            ]
        )
        pos_rows.append(
            {"Position": pos_label, "Count": count, "Status": "✓" if count > 0 else "✗"}
        )
    st.dataframe(pd.DataFrame(pos_rows), use_container_width=True, hide_index=True)

with tab2:
    center_imgs = [
        img for img in selected_piece_images if img.get("object_position") == "center"
    ]
    combo_rows = []
    for intensity in intensities:
        for temp in color_temps:
            exists = any(
                img.get("light_intensity") == intensity
                and img.get("color_temp") == temp
                for img in center_imgs
            )
            combo_rows.append(
                {"Intensity": intensity, "Temp": temp, "Status": "✓" if exists else "✗"}
            )
    st.dataframe(pd.DataFrame(combo_rows), use_container_width=True, hide_index=True)

with tab3:
    random_imgs = [
        img for img in selected_piece_images if img.get("object_position") == "random"
    ]
    ...
    if random_imgs:
        ...
        st.dataframe(
            pd.DataFrame(random_rows), use_container_width=True, hide_index=True
        )
