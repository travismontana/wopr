...
from lib.basic_functions import debugit, get_config, setup_logger
from lib.model_family_lib import create_new_model_family, get_model_family
from lib.models_lib import create_new_model, get_models, update_model_info
...
def create_model_page():
    ...
    with st.form("create_model_form"):
        ...
        model["family"] = st.selectbox(
            "Model Family", options=model_families, format_func=lambda x: x["name"]
        )
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "version": st.column_config.NumberColumn(
                "Version", min_value=1, max_value=100, step=1, default=1, format="%d"
            ),
    ...
    if not edited_df.equals(df):
        ...
        if not new_rows.empty:
            for _, row in new_rows.iterrows():
                ...
                create_result = create_new_model(
                    model_data.to_dict(orient="records"), debug
                )
...
def create_model_family_page():
    ...
    with st.form("create_model_family_form", enter_to_submit=False):
        ...
        submitted = st.form_submit_button(
            "Create Model Family", key="create_model_family_submit"
        )
    ...
    if submitted:
        ...
        if st.session_state.debug:
            ...
            st.write(
                f"({st.session_state.debuggers['create_model_family_page']})<-shouldbedata"
            )
...
with st.sidebar, st.expander("Settings"):
    # Debug toggle
...
try:
    ...
    if e.response.status_code == 503:
        if st.session_state.attempts < 3:
            ...
            with st.spinner(
                "Waiting for API to load...will retry 3 times every 60 seconds",
                show_time=True,
            ):
                time.sleep(60)
            st.rerun()
