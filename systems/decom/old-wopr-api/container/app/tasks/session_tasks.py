...
def _get_session_data(session_id: str) -> tuple[dict, str]:
    ...
    logger.info(
        f"Session data retrieved: session_id={session_id}, session_uuid={session_uuid}"
    )
...
def _get_session_filenames(session_id: str) -> list[str]:
    ...
    logger.info(
        f"Retrieved {len(session_plays) if session_plays else 0} play records from playtracker"
    )
    ...
    filenames = [play.get("filename") for play in session_plays if play.get("filename")]
    ...
    logger.info(
        f"=== _get_session_filenames END === session_id={session_id}, filename_count={len(filenames)}, filenames={filenames}"
    )
    return filenames
...
def archive_session_images(session_id: str) -> dict[str, list[dict[str, str]]]:
    ...
    logger.info(
        f"Paths: base_path={base_path}, archive_base_path={archive_base_path}, incoming_path={incoming_path}, archive_path={archive_path}"
    )

    # Get list of files to archive
    files_to_archive = _get_session_filenames(session_id)
    logger.info(
        f"Files to archive: count={len(files_to_archive)}, files={files_to_archive}"
    )
    ...
    failures = []
    logger.info(
        f"Beginning file archiving operations for {len(files_to_archive)} files"
    )
    ...
    logger.info(
        f"=== archive_session_images END === session_id={session_id}, archived_count={len(results)}, failed_count={len(failures)}"
    )
...
def check_session_file_status_task(session_id: str) -> dict[str, list[str]]:
    ...
    # Initialize SafeFS wrapper
    filesafe = SafeFS(
        base_dir=woprvar.storage_paths["wopr_base_path"], forbid_symlinks=True
    )
    logger.info(
        f"SafeFS initialized with base_dir={woprvar.storage_paths['wopr_base_path']}, forbid_symlinks=True"
    )
    ...
    presence = {key: [] for key in locations}
    logger.info(
        f"Beginning file presence check across "
        ...
        for location_key, location_path in locations.items():
            ...
            try:
                ...
                resolved = filesafe._resolve_rel(
                    str(file_path.relative_to(wopr_base_path)), must_exist=False
                )
                if resolved.exists():
                ...
                logger.warning(f"Error checking {filename} in {location_key}: {e}")
                continue
    ...
    logger.info(
        f"=== check_session_file_status_task END === session_id={session_id}, presence={presence}"
    )
...
def copy_files_to_label_source(session_id: str) -> dict[str, list[dict[str, str]]]:
    ...
    label_source_path = (woprvar.storage_paths.get("label_source_path")).resolve()
    logger.info(
        f"Computed label paths: label_base_path={label_base_path}, label_source_path={label_source_path}"
    )
    ...
    logger.info(
        f"SafeFS initialized with base_dir={wopr_base_path}, forbid_symlinks=True"
    )
    ...
    try:
        ...
        filesafe.mkdir(
            str(label_source_path.relative_to(wopr_base_path)), exist_ok=True
        )
        logger.info(f"Label source directory created/verified at {label_source_path}")
    except (OSError, ValueError, RuntimeError) as e:
        logger.error(
            f"Failed to create label source directory {label_source_path}: {e}"
        )
        raise
    ...
    for filename in files_to_copy:
        ...
        try:
            ...
            logger.info(
                f"File copied to label source successfully: {filename}, method={how}"
            )
            results.append(
                {
                    "filename": filename,
    ...
    logger.info(
        f"=== copy_files_to_label_source END === session_id={session_id}, success_count={len(results)}, failed_count={len(failures)}"
    )
