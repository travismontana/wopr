...
def queue_task(task_func, *args, **kwargs) -> dict[str, Any]:
    ...
    try:
        ...
        return {"task_id": task.id, "task_name": task_func.name, "status": "queued"}
...
def get_task_status(task_id: str) -> dict[str, Any]:
    ...
    response = {"task_id": task_id, "state": task.state, "status": task.state.lower()}
...
def wait_for_task(task_id: str, timeout: float | None = None) -> dict[str, Any]:
    """
...
def get_all_tasks(filter_state: str | None = None) -> dict[str, Any]:
    """
    ...
    try:
        ...
        total_count = sum(
            len(worker_tasks) if isinstance(worker_tasks, dict) else 0
            for worker_tasks in tasks.values()
        )
        logger.info(
            f"Retrieved all task states from Celery workers (total workers: {total_count})"
        )
        return tasks
