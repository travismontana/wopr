...
CAPTURE_RESPONSE_HEADERS = ["content-type", "content-length", "cache-control"]
...
app.include_router(
    notifications.router, prefix="/api/v2/notifications", tags=["notifications"]
)
...
app.include_router(
    model_family.model_family_router,
    prefix="/api/v2/model_family",
    tags=["model_family"],
)
...
async def capture_headers_and_payloads(request, call_next):
    ...
    logger.debug(
        f"[MIDDLEWARE] Span recording: {span.is_recording() if span else False}"
    )
    ...
    if span and span.is_recording():
        ...
        try:
            async for chunk in response.body_iterator:
                response_body += chunk
            logger.debug(
                f"[MIDDLEWARE] Captured response body length: {len(response_body)}"
            )
        except Exception as e:
        ...
        if response_body:
            try:
                ...
                try:
                    span.set_attribute(
                        "http.response.body", response_body.decode()[:1000]
                    )
                except UnicodeDecodeError:
                    # Binary response (image, etc.)
                    span.set_attribute(
                        "http.response.body",
                        f"<binary data, {len(response_body)} bytes>",
                    )
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Starting uvicorn server on {woprvar.SERVICE_HOST}:{woprvar.SERVICE_PORT}"
    )
