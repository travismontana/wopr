...
if not LABEL_STUDIO_TOKEN:
    ...
    logger.info("Label Studio token found - vision endpoints enabled")
...
async def list_projects() -> ProjectListResponse:
    ...
    """
    with (
        tracer.start_as_current_span("vision.list_projects")
        if tracer
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.get(url, headers=get_ls_headers())
            ...
            logger.error(f"Failed to reach Label Studio: {e!s}")
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail=f"Failed to communicate with Label Studio: {e!s}",
            )
...
async def get_project(project_id: int) -> dict[str, Any]:
    ...
    """
    with (
        tracer.start_as_current_span("vision.get_project")
        if tracer
        else nullcontext() as span
    ):
        if span and span.is_recording():
...
async def create_task(request: TaskCreateRequest) -> TaskCreateResponse:
    ...
    """
    with (
        tracer.start_as_current_span("vision.create_task")
        if tracer
        else nullcontext() as span
    ):
        if span and span.is_recording():
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.post(
                    url, headers=get_ls_headers(), json=payload
                )
                response.raise_for_status()
                task_data = response.json()

                logger.info(
                    f"Created task {task_data.get('id')} in project {request.project_id}"
                )
...
async def health_check() -> dict[str, str]:
    ...
    with tracer.start_as_current_span("vision.health") if tracer else nullcontext():
        ...
        try:
            ...
            logger.error(f"Label Studio health check failed: {e!s}")
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                detail=f"Label Studio unavailable: {e!s}",
            )
...
async def list_tasks(project_id: int) -> list[dict[str, Any]]:
    ...
    """
    with (
        tracer.start_as_current_span("vision.list_tasks")
        if tracer
        else nullcontext() as span
    ):
        if span and span.is_recording():
