  ...
  """
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Error capturing piece image, error: {e}",
        )
    ...
    if not image_uuid:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="No UUID found in mlimage data",
        )
    FILENAME = f"{piece_id}-{game_catalog_id}-{image_uuid}.jpg"
    THUMBNAME = f"{piece_id}-{game_catalog_id}-{image_uuid}-thumb.jpg"
    payload = {
        "filenames": {"fullImageFilename": FILENAME, "thumbImageFilename": THUMBNAME}
    }

    # Update directus record with filenames
    try:
        response = requests.patch(
            f"{URL}/{image_id}", json=payload, headers=woprvar.DIRECTUS_HEADERS
        )
        response.raise_for_status()
        logger.info(
            "Successfully updated piece filename, response: %s", response.json()
        )
    except requests.RequestException as e:
        logger.error(f"Error capturing piece image: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error setting filename for piece image, error: {e}",
        )
