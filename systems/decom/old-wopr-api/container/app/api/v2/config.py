...
DIRECTUS_URL = os.getenv("DIRECTUS_URL", "http://wopr-directus:8055")
...
async def get_all(environment: str = "production"):
    ...
    with (
        tracer.start_as_current_span("config.get_all")
        if tracer
        ...
        params = {"filter[environment][_eq]": environment, "fields": "data"}
        ...
        try:
            ...
            raise HTTPException(
                status_code=503, detail=f"Failed to fetch config from Directus: {e!s}"
            )
            ...
            raise HTTPException(
                status_code=500, detail="Invalid response from Directus API"
            )
...
async def get_environments():
    ...
    # Start custom span for better tracing granularity
    with (
        tracer.start_as_current_span("config.get_environments")
        if tracer
        else nullcontext() as span
    ):
        if span and span.is_recording():
        ...
        logger.info("Fetching all environments config")

        # Build Directus API request
        url = f"{DIRECTUS_URL}/items/woprconfig"
        params = {"fields": "environment"}
        ...
        try:
            ...
            if not result.get("data") or len(result["data"]) == 0:
                if span and span.is_recording():
                    span.set_attribute("config.found", False)
                raise HTTPException(
                    status_code=404, detail=f"No configuration found for data: {data}"
                )
