...
from wopr.config import get_bool, get_list, get_str
...
def imagefilename(
    ...
    if subject not in allowed_subjects:
        raise ValueError(
            f"Invalid subject '{subject}'. Must be one of: {allowed_subjects}"
        )
    ...
    if extension not in allowed_extensions:
        raise ValueError(
            f"Invalid extension '{extension}'. Must be one of: {allowed_extensions}"
        )
...
def thumbnailfilename(game_id: str, subject: str) -> str:
    ...
    filename = template.format(
        timestamp=timestamp, subject=subject, game_id=game_id, extension=extension
    )
...
def list_game_images(game_id: str, extension: str | None = None) -> list:
    """
