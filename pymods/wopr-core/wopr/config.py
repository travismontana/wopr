...
class ConfigClient:
    ...
    def __init__(self, service_url: str | None = None, timeout: int = 5):
        """
        ...
        if service_url is None:
            service_url = os.getenv("WOPR_API_URL", "https://wopr-api/api/v2/config")
    ...
    def get(self, key: str, default: Any = None, use_cache: bool = True) -> Any:
        ...
        try:
            response = requests.get(
                f"{self.service_url}/get/{key}", timeout=self.timeout
            )
    ...
    def get_multiple(self, keys: list) -> dict:
        ...
        try:
            response = requests.post(
                f"{self.service_url}/get", json={"keys": keys}, timeout=self.timeout
            )
            response.raise_for_status()
            data = response.json()
    ...
    def get_section(self, section: str) -> dict:
        ...
        try:
            response = requests.get(
                f"{self.service_url}/section/{section}", timeout=self.timeout
            )
    ...
    def reload(self) -> None:
        ...
        try:
            response = requests.post(f"{self.service_url}/reload", timeout=self.timeout)
            response.raise_for_status()
            self.clear_cache()
...
_client: ConfigClient | None = None


def init_config(service_url: str | None = None, timeout: int = 5) -> None:
    """
    ...
    global _client
    if not service_url:
        service_url = os.getenv("WOPR_API_URL", "")
